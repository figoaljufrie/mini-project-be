...
export const getTransactionsByOrganizer = async (
  req: AuthenticatedRequest,
  res: Response
) => {
  try {
    ...
    const result = await transactionService.getTransactionsByOrganizer(
      organizerId,
      Number(page),
      Number(limit)
    );
    ...
    return res.json({
      success: true,
      message: "Transactions for your events retrieved successfully",
      data: result,
    });
  } catch (error) {
...
export const updateTransactionStatus = async (
  ...
  try {
    ...
    // Pastikan organizer hanya bisa update transaksi untuk event mereka sendiri
    const transaction = await transactionService.getTransactionById(
      transactionId
    );
    if (!transaction) {
      throw new Error(`Transaction dengan ID ${transactionId} tidak ditemukan`);
    }

    if (transaction.event.organizer.id !== req.user.id) {
      return res.status(403).json({
        success: false,
        message:
          "Access denied. Hanya bisa mengupdate transaksi untuk event sendiri.",
      });
    }
...
export const cancelTransaction = async (
  ...
  try {
    ...
    const canceledTransaction = await transactionService.cancelTransaction(
      transactionId,
      userId
    );
    ...
    if (
      ...
      error.message.includes("tidak dapat dibatalkan")
    ) {
      return res.status(400).json({
        success: false,
        message: error.message,
...
export const getExpiringTransactions = async (req: Request, res: Response) => {
  try {
    ...
    const transactions = await transactionService.getExpiringTransactions(
      limitNumber
    );
